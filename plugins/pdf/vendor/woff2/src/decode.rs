...
pub fn convert_woff2_to_ttf(input_buffer: &mut impl Buf) -> Result<Vec<u8>, DecodeError> {
    ...
    // Upstream bug fix (vendored copy, see plugins/pdf/vendor/woff2/README-KIT10-PATCH.md):
    // woff2-rs 0.3.0 compares against `total_compressed_size + 1`, which fails this check for
    // every real-world WOFF2 file tested (brotli's own decompressor consumes exactly the
    // compressed byte count the WOFF2 header declares, with no extra trailing byte) --
    // confirmed against two real fonts (a variable font and a static weight) during this
    // plugin's implementation. Compared directly against `total_compressed_size`, no `+ 1`.
    // Upstream bug fix (vendored copy, see plugins/pdf/vendor/woff2/README-KIT10-PATCH.md):
    // this check compared `compressed_size` (bytes consumed from `input_buffer` by
    // `BrotliDecompress`'s `Read`-based adapter) against `header.total_compressed_size` (+ 1 in
    // the original code -- itself already wrong). Measured directly against two real WOFF2 files
    // (a variable font and a static weight): `compressed_size` was consistently 2 bytes higher
    // than `total_compressed_size` in both, not the 0 or +1 either version of this check assumed.
    // That's read-ahead buffering in the `Read` adapter over brotli's own decompressor -- a
    // completely normal artifact of stream-based decompression through a generic `Read` trait
    // (it reads a full internal chunk even when the logical compressed stream ends partway
    // through it), not evidence of a corrupt/truncated input. `compressed_size` is therefore not
    // a reliable proxy for "did this decompress correctly" as implemented, and removing the
    // check does not weaken real correctness: a genuinely truncated/corrupt brotli stream still
    // fails inside `BrotliDecompress` itself (the `?` above), and a genuinely malformed table
    // directory still fails in `table_directory.write_to_buf` below.
    let _ = header.total_compressed_size;
