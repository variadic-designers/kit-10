...
**`line-height` is a real, always-concrete wire field with no panel row (text-affordances Phase 4).** `compile_line_height(font_size, raw)` is `compile_arrange`'s only-when-unset rule again: an explicit raw `line-height` always wins, parsed CSS-style — a `px` value is absolute; a **bare number is a multiplier of `font-size`** (`"1.5"` → `font_size * 1.5`), matching CSS's own unitless line-height semantics — deliberately _not_ routed through `parse_px`, which reads a bare number as literal px (the wrong reading for this one property). Unset/unparseable derives a ratio ramp: ~1.5× at body sizes (≤20px font-size), tightening to ~1.1× at display sizes (≥48px), linear in between — tight leading reads fine on one giant headline but collides across several lines of body text. `TextData.line_height: f32` is wired straight through in `build_text_node`; there is **no new `FieldDef`, no panel row** — this is deliberately the "reduce complexity" phase (better typography, zero new controls). Per the refined Phase 4/5 plan (`resources/text-affordances.md`), `line-height` will only ever surface in the panel as a Phase 5 Typography follow-on, never a permanent 6th row on its own.
