...
- **`viewCompositionKeys` (Editor.svelte) is ACCUMULATED, not per-frame.** It's the plugin-declared composition field keys (`inputType: 'children'`), but `fieldCategories` is per-active-view (box vs text), so it's merged into a growing set — once `children` is seen (any box view resolved) it stays known, so selecting a text/no-kit view never empties it and flattens the Views tree.
