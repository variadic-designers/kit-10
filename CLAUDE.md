...
- Size fields are `Extent` (`Auto | Px(f32) | Percent(f32)`), not bare `f32`. `Box{width: Auto}` auto-sizes to children+padding; `max_*: Auto` means no constraint; `min_*` is a lower clamp (Box and Shape only - Shape mirrors Box's own sizing box exactly).
