...
- **Writing a per-view override (e.g. one view's `children`) goes through `api.upsertViewToken(projectId, viewId, alias, value)` — never through the resolved property's `tokenId`.** A View token overrides a same-named Kit/Project token during resolution (CONCEPTS.md §Tokens: "a View token overrides a Kit token of the same name"), matched by **alias** in `resolve.ts`'s `viewListTokenMap`/`tokenMap`. But a resolved property's `tokenId` is the *declaring render entry's* token, which for a kit-declared property (like the seed's `children`, whose only entry sits on the kit null layer pointing at a shared kit-scoped base token) is that **shared base**, not this view's override — writing it either moves every view at once or is silently shadowed by the per-view token and does nothing. `upsertViewToken` (find-or-create the view-scoped token for the alias, in one transaction) is the shared write path used by **both** `ChildViewField.svelte` and the Views panel's drag-and-drop nesting (`Views.svelte`'s `setViewChildren`); a render entry is minted only when nothing token-declares the property yet. This exact bug hit both call sites before they shared the helper.
