...
**Utility plugins (`loadUtilityPlugin`/`callUtilityPlugin`):** a second, separate plugin category alongside Charter's viewport-interpreter lifecycle. `activePlugin` is a single variable — `loadPlugin` unconditionally overwrites it, so a second `loadPlugin` call would silently evict Charter. Utility plugins are stored in their own `Map<string, Plugin>` instead, never touch `activePlugin`/`on_resolve`/`on_selection_change`/`on_field_update`, and are only ever called on demand by name (`callUtilityPlugin(name, fn, payload)`). **Fontavious** (`plugins/fontavious/`) is the first one — it owns a bare, static font catalogue and fetches WOFF2 files from vendor CDNs via Extism's built-in HTTP capability (`allowedHosts` in the `loadUtilityPlugin` options, e.g. `['fonts.gstatic.com']` — requires `runInWorker: true`, already the default here), handing the resulting bytes to `vellum.load_font()`. Its four exports: `search_fonts` (catalogue substring search, for `SuggestField.svelte`), `fetch_font` (returns raw bytes, not JSON — the caller reads them via the JS SDK's `.bytes()`), `variant_url` (JSON in/out, no HTTP — resolves a `{value, weight, style}` request to the URL `fetch_font` would use, without fetching it), and `family_facts` (JSON in/out, no HTTP — a family's variant weight ranges + styles, minus URLs; the input to Charter's weight snapping, see the font-facts note below).

**`Editor.svelte`'s resolve-time font scan asks Fontavious "which URL", never Vellum "is this weight loaded".** Whether two different weights of a family are the same file (a variable font, one URL covers a range) or genuinely different files (a static font, one URL per weight) is a fact Fontavious's catalogue already has — `weightMin`/`weightMax` per variant. Vellum used to be asked to answer this by introspecting a loaded font's actual variable-axis range (via `swash`, briefly a direct dependency); that duplicated a fact one layer up already knew, and made Vellum's font-loading API font-catalogue-shaped instead of just "load bytes, render them". Now the scan calls `variant_url` (cheap, no I/O) to get the URL a `(family, weight)` request would resolve to, and keeps its own plain `Set<string>` of URLs already fetched+loaded (plus an in-flight set — several weights of a variable font resolve to one URL, and each would otherwise launch its own duplicate download in the same burst) — same URL as something already loaded means skip, different URL means fetch. Vellum's only font-membership export is back to the family-only `is_font_loaded`.

**Charter snaps text weights to what the family actually has (`resolve_font_weight` — text-affordances Phase 1, UX-scoped).** `Editor.svelte` fetches `family_facts` once per family named in resolved data and hands the assembled map to `setData` → `on_resolve`'s `fontFacts` input (camelCase, JS-authored; rides `last_resolve_input`, so the selection fast path snaps identically). At the end of `build_viewport`, `snap_text_weights` applies the CSS font-weight matching algorithm per Text node, so a requested weight the family doesn't ship (Lato 600) deterministically renders the nearest real one (700) — the stored kit value is never rewritten; substitution is a render decision. `OnResolveResult` gains `font_requests` (snake_case, Charter-authored): the concrete post-snapping `(family, weight, style)` set, which is what the editor's scan now iterates instead of re-deriving weights from raw kit properties — **one decision point; the scan is its supply chain**. Two cautions: facts are NOT DB rows, so the `rowsKey` dedup never sees them — `fontFacts` is a Svelte `$state` reassigned (never mutated) so its arrival re-fires the `setData` effect and forces the re-resolve; and a resolve can legitimately run before a family's facts land (font_requests briefly carries the unsnapped weight, `variant_url` warns "no variant" once), self-correcting on the facts re-resolve. Deliberately catalogue-only for now: the Vellum `font_facts`-from-loaded-bytes oracle (for uploaded fonts) is deferred until uploads exist — see `resources/text-affordances.md`.

**`font-weight` is `inputType: "weight"` — a pick-from-what-exists segmented control, not a free-typed number (text-affordances Phase 2, partially shipped).** `WeightField.svelte` renders the nine standard named weights (Thin 100 … Black 900) filtered to what the resolved family's `fontFacts` entry covers — Lato shows only `[400, 700]`; an uncatalogued family (no facts entry) falls back to the full unfiltered set, Charter's own "no facts, no opinion" rule applied client-side. Unlike `arrangeKeys`/`resizeKeys`, Charter declares no companion data for this field — the choices are runtime facts it doesn't hold, so it only marks the field's kind. Not shipped: a raw-numeric escape hatch — `font-weight` has no separate raw property to expose the way Arrangement's Advanced row exposes `flex-direction` alongside its friendly control, so the picker is the only panel surface now (parsing itself is untouched; DB/import/seed values still resolve and still get Phase 1's snap, just aren't typable from this panel).

**`text-align`/`text-decoration` are real, no longer dead-end fields (text-affordances Phase 3).** `parse_text_align`/`parse_text_decoration` (Charter) map kit-property keywords (`"center"`/`"right"`/`"justify"`, `"underline"`/`"line-through"`) to the exact Vellum enum variant names (`TextAlign`/`TextDecorationKind`, no `rename_all`, same convention as `font_style`'s hardcoded `"Normal"`) — same default-hard fallback grammar as `parse_arrange` (absent/unrecognized reads as `Left`/`None`). `inputType: "align"`/`"decoration"` render as four- and three-icon segmented rows in `StyleField.svelte`, inline (no wrapper component) — fixed, Charter-known choice sets like `resize`'s Fixed/Hug/Fill, not runtime facts like `weight`. The shared segmented-button CSS (`.seg-control`/`.seg-control__btn`) was generalized from `resize`'s formerly resize-only class names once a third field needed the identical look.

**`line-height` is a real, always-concrete wire field (text-affordances Phase 4).** `compile_line_height(font_size, raw)` is `compile_arrange`'s only-when-unset rule again: an explicit raw `line-height` always wins, parsed CSS-style — a `px` value is absolute; a **bare number is a multiplier of `font-size`** (`"1.5"` → `font_size * 1.5`), matching CSS's own unitless line-height semantics — deliberately _not_ routed through `parse_px`, which reads a bare number as literal px (the wrong reading for this one property). Unset/unparseable derives a ratio ramp: ~1.5× at body sizes (≤20px font-size), tightening to ~1.1× at display sizes (≥48px), linear in between — tight leading reads fine on one giant headline but collides across several lines of body text. `TextData.line_height: f32` is wired straight through in `build_text_node`.

**Panel surface: `FieldDef::new("line-height", Some("Leading"))` with deliberately no `inputType`** — plain free text, same as `font-size`, not a numeric stepper. A stepper (e.g. the `spacing` scalar stepper `gap`/`cell-min` use) always writes a bare number, which would silently collide with `compile_line_height`'s CSS-style dual reading (bare = multiplier, `px` = absolute) — free text is what lets a designer type either form directly. Originally planned to have **no** standalone row at all (only ever a Phase 5 Typography follow-on) — shipped as a standalone field instead per explicit user request to surface it ahead of Phase 5, which remains deferred. When Phase 5 lands, this `FieldDef` is expected to fold into `typographyKeys` rather than staying a 6th top-level row.
...
**Fontavious's catalogue (`plugins/fontavious/catalogue.json`) models weight as a range** (`weightMin`/`weightMax` per variant), not a single `weight` — a variable font genuinely covers a continuous range from _one_ URL (confirmed against Google Fonts' live CSS2 API: requesting `wght@400..700` returns one `font-weight: 400 700;` face for a true variable font, vs. discrete single-weight faces for a static-only one), and a static font is just the degenerate `weightMin == weightMax` case. Most catalogued families are variable; a few (Lato, Poppins, confirmed per-family against the live API, not assumed) have no variable version and keep one URL per discrete weight. (Vellum once had an `is_font_variant_loaded(name, weight)` export that introspected a loaded font's actual variable-axis range via `swash` to answer per-weight questions — removed when the scan moved to `variant_url`; the range facts now flow from the catalogue via `family_facts` instead, and Vellum answers only family-level `is_font_loaded`.)

**Not every font has every weight, even for a catalogued family.** Lato and Poppins, for example, only ship 400/700 from Google Fonts — no in-between weight like 600 exists to fetch, variable or otherwise. Since the weight-snapping note above, Charter handles this deterministically (`resolve_font_weight` snaps 600 → 700 once the family's facts have landed); a `fetch_font`/`variant_url` "no variant" error can still occur in the brief pre-facts window or for an uncatalogued family, is logged via `console.warn`, and is never surfaced as a user-facing error — a real, permanent limitation of the font, not a bug to fix. cosmic-text's own font matching remains the last-line fallback during real shaping: it substitutes the closest already-loaded weight of the same family rather than falling through to an unrelated font.

**Size fields are an `Extent` enum, not bare `f32` (`Auto | Px(f32) | Percent(f32)`).** `Box` carries `width`/`height`/`min_width`/`min_height`/`max_width`/`max_height`; `Text`/`Img` carry `width`/`height`. This mirrors taffy's `Dimension` and is the same wire-enum pattern as `TrackSize`. `Extent::Auto` is the old `width:0`-means-auto default; `Percent` is fraction-of-parent (`0.5` == `50%`), resolving to auto against an auto-sized parent (standard CSS). `fr` is deliberately NOT an `Extent` — fractional-of-free-space is a container concern (grid `TrackSize::Fr` / flex `flex_grow`), never a self-declared size.

- `Box{width: Auto}` → Vellum auto-sizes this box to its children + padding.
- `Text{width: Auto, height: Auto}` → Vellum measures during layout (cosmic-text, constrained by parent available space). Charter must always emit `Auto` for text dimensions.
- `min_width`/`min_height` (Box only) give percent widths a usable floor (a percent of an auto parent otherwise collapses to 0).
...
| `kit10_panel_publish`               | Plugin publishes a `PanelManifest` keyed by `panel_id` into the editor's `$state` map; panels derive off `pluginManager.panelManifest(id)`. Charter uses this from inside `on_resolve`'s body to ship the Views panel topology; a future non-Charter plugin can use the same channel to drive its own panel on whatever cadence it wants. Always assigns a new `Map` instance to trigger Svelte 5 reactivity (never mutates in place). |
...
**Per-axis resizing (Fixed / Hug / Fill) is Charter's first real sizing opinion (`compile_resize`).** `width`/`height` accept the keywords `fill` and `hug` (alongside a length/`%`/`auto`). Charter compiles them to taffy primitives so a designer never hand-wires flex: **Fill** → `flex-grow:1`, `flex-shrink:1`, `flex-basis:0`, `min:0` on the parent's MAIN axis (equal share, can shrink), `align-self: stretch` on the CROSS axis; **Hug** → `flex-grow:0`/`flex-shrink:0` (content size, may overflow — exactly Figma's Hug); a length is Fixed. It's **direction-aware** — `render_view_nodes` threads the parent box's main axis (`parent_main_horizontal: Option<bool>`) down; `None` (a top-level view on the infinite canvas) has no flex parent, so Fill degrades to plain auto. Only an explicit `fill`/`hug` keyword engages compilation — a plain length/absent width keeps prior CSS behavior, so existing layouts don't shift, and the raw item-flex props remain a parse-time escape hatch. The editor renders this via `FieldDef` `inputType: "resize"` → a Fixed/Hug/Fill segmented control in `StyleField.svelte` that writes exactly these keyword values (so panel and compiler share one contract). This is the concrete example of the "opinionated translation layer" note above; the Fixed/Hug/Fill control replaced the temporary raw `flex-grow`/`flex-shrink`/`align-self` fields.

**Size limits (min/max) ride the resize control as contextual follow-ons (`ResizeKeys`), not top-level rows (layout-affordances Phase 4).** The four raw `min-`/`max-width`/`height` `FieldDef`s are gone from `box_categories()`'s top level; each `resize` field (`width`/`height`) instead carries its own limit pair in a `resizeKeys` side-channel (`min`/`max`, full `FieldDef`s — same "typed side-channel keyed by inputType" shape as `arrangeKeys`). `ResizeField.svelte` wraps the plain resize `StyleField` and reveals the pair inline exactly when a limit is meaningful — the dimension is Fill or a fixed `%` (the percent-of-auto-parent floor case — a `%` of an auto parent collapses to 0 without a `min`, see the Size-fields `Extent` note above) — OR when either limit already has a value (a set limit always clamps in Vellum regardless of mode, so it must never be invisible). The properties themselves still parse identically in Charter (`parse_extent` reads them off the props map, independent of `FieldDef` placement); only the panel surface moved.

**Arrangement (Stack / Cluster / Split / Center / Grid) is Charter's second layout opinion (`compile_arrange`), documented in full in `resources/layout-affordances.md`.** `box_categories()`'s `layout` category shrank from ~19 fields to 4: `arrange` (`inputType: "arrange"`), `width`/`height` (`resize`, each carrying its min/max pair via `resizeKeys` — see the size-limits note above), and `padding` (`inputType: "spacing"`, `spacingMode: "box"`). The raw arrangement knobs (`flex-direction`, `gap`, `align-items`, `justify-content`, `flex-wrap`, `display`, the `grid-template-*`/`grid-auto-*`/`grid-column`/`grid-row` sublanguage, plus the new `grid-cell-min`) are no longer top-level `FieldDef`s at all — they exist only nested inside the `arrange` field's `arrangeKeys` (`direction_key`, `gap`, `cell_min`, `advanced`, `grid_advanced`), the same "typed side-channel keyed by inputType" shape `suggestions_from` already established. This keeps the editor from ever hardcoding a companion property name: `ArrangeField.svelte` reads `field.arrangeKeys` as data and renders each companion via a plain `<StyleField>` — it doesn't know the strings "flex-direction" or "gap" exist. The editor renders `arrange` as a tab row (Stack/Cluster/Split/Center/Grid) + an inline submenu of follow-on fields, plus a per-tab `<details>` disclosure ("Advanced flex" / Grid's "Custom tracks") holding the raw escape hatches — never a popover/context menu, matching `resize`'s established inline-disclosure grammar.

`compile_arrange` follows `compile_resize`'s own convention exactly: every output is a **default that only fires when the corresponding raw property was never explicitly set** (via its tab's own follow-on OR the Advanced/Custom-tracks escape hatch) — never an unconditional force. This is deliberate: forcing would make an Advanced field's edits silently have no visible effect while a non-Stack tab is active, which is the exact "a control's meaning changes for a reason the panel doesn't show" failure the whole feature exists to avoid. Concretely: **Stack** only defaults `align-items: Center` when direction resolves to Row and align-items is unset (direction itself is a free follow-on choice, not a default); **Cluster** defaults `flex-direction: Row`, `flex-wrap: Wrap`, `align-items: FlexStart` (Cluster has no Direction follow-on — row+wrap IS its identity); **Split** defaults `flex-direction: Row`, `justify-content: SpaceBetween`, `align-items: Center`; **Center** defaults `justify-content`/`align-items: Center` and never touches direction (axis-free); **Grid** defaults `grid-template-columns: [TrackSize::AutoFit(cell_min)]` only when the raw `grid-template-columns` (Custom tracks) was never set — Cell-min alone produces a responsive grid the instant Grid is picked, Custom tracks is the override. `resolve_flex_direction` centralizes the old inline row/row-reverse/column-reverse/else-column match plus each kind's own directional default (Cluster/Split → Row, else → Column) into one place `compile_arrange` calls, replacing the duplicate match that used to live directly in `build_box_node`.

**`TrackSize` gained an `AutoFit(f32)` variant — one opinionated shape, not general CSS `repeat()`.** It's exactly `repeat(auto-fit, minmax(<f32 px>, 1fr))`, the one thing Grid's Cell-min control needs; Charter never emits `auto-fill` or an arbitrary repeat count. Mirrored exactly in Vellum (`taf_can_do/src/api.rs`), same "must match exactly" rule as every other `TrackSize` variant. `taf_can_do/src/layout/mod.rs`'s `track_to_template` maps it to taffy's `GridTemplateComponent::Repeat` (via `style_helpers::repeat(RepetitionCount::AutoFit, vec![minmax(length(n), fr(1.0))])`) — a different `GridTemplateComponent` variant than the `Single` every other `TrackSize` produces, so it can't go through the shared `sized: TrackSizingFunction` + `.into()` path the others share. `track_to_non_repeated` (grid-auto-rows/columns, which can't repeat) degenerates it to a plain non-repeated `minmax`.

**`extract_paint_props`'s `padding` now parses real CSS 1/2/3/4-value shorthand (`parse_padding_shorthand`), not a single uniform value.** Order is `[top, right, bottom, left]`, same as `BoxData.padding`/`TextData.padding` always were. This is what backs the `spacing` inputType's box-mode 1↔4 expand/collapse control in the Render panel (`StyleField.svelte`) — the stored value literally is the shorthand string ("12", "8 16", "4 8 12", or "1 2 3 4"), and which layout (one stepper vs. four) the widget shows is _derived from the value itself_ (how many space-separated numbers it holds), never separate component state. Applies to `Text` too since it shares `extract_paint_props` with `Box` — not a separate change. Token-scale-aware snapping (nudging toward a project-defined spacing scale) is a natural future extension once KIT•10 has a spacing-scale token type; it doesn't yet (only `scalar`/`view`/`view-list` token values exist), so this ships as a plain numeric stepper for now.

**The `Img` primitive carries `fit: String` (object-fit), never a `cover: bool`.** Charter emits `fit` (`"cover"|"contain"|"fill"`, unknown → `cover`) as the exact field name Vellum reads; an earlier `cover: bool` was an unknown key Vellum silently dropped, so every image rendered as the `fit` default. `object-fit: cover` (and any oversized image) is **clipped to the node box** CPU-side in `prepare_images` (geometry+UV intersection; `ImageInstance` carries `uv_min`/`uv_max`, the image shader lerps UV) — there is no scissor in that pass, so without the clip a cover image bleeds outside its bounds. Intrinsic sizing + aspect-ratio (`measure_img_node`) is browser-correct; `min`/`max` width/height are Box-only, so images can't yet express `max-width: 100%`.
...
| `on_resolve`          | Data change           | `OnResolveInput`                     | `OnResolveResult` (viewport + categories); also publishes `PanelManifest("views")` via `kit10_panel_publish` host fn (see Panel Manifests below) |
...
**The Views-tree icon lives in `hints.view_icon`, not in the manifest.** The editor reads the icon straight from `viewsQuery` rows (`v.hints?.view_icon`), falling back to `fa-regular fa-window-maximize` — Charter never emits icon strings for the panel. The seed sets `view_icon` alongside `charter.primitive` at each view-creation site. The `viewLocked → fa-lock` override stays editor-side — that's editor state, not a primitive.

**Panel Manifests (Charter → editor: composition key + per-view facts).** The manifest ships a separate channel, decoupled from `OnResolveResult`'s return shape, via the host fn `kit10_panel_publish(panel_id, manifest)`, routed into a Svelte `$state` map keyed by `panel_id`. `build_views_panel_manifest` bundles, per `PanelItem`: `id` (view_id today, opaque to the editor), `write_alias` (the token alias to upsert on DnD), and `ops` (the context-menu op set the plugin declares available on this item). The manifest also carries `header_ops` for the panel-header affordance (the `+` menu) and `composition_field_keys` (the resolved-property name(s) Charter treats as composition fields — its single nesting opinion). The manifest does **not** carry `child_ids` or `is_root` — tree topology is computed host-side from `resolvedViews` × `composition_field_keys`, because that's a generic graph walk the host is perfectly positioned to do over already-resolved data; routing it through the plugin just wrapped a universal computation across the WASM boundary for nothing. It also does **not** carry `icon` — the panel reads that directly from `hints.view_icon`. Live metadata (view name, locked/hidden state) stays in the editor's own `viewsQuery` DB rows and is joined against the manifest's `id` at render time, so a rename fires through the live DB query without the plugin needing to re-publish.

**The nesting split (Phase 3):** Charter's opinion on nesting is _one string list_ (`composition_field_keys`: "this field's `viewRefs` are the children"). The host owns everything else: walking `resolvedViews` for those keys' `viewRefs`, building `childrenByViewId`, partitioning root vs. referenced, rendering the tree, and cycle-guarding via `isDescendant`. This restores the pre-Phase-1 layering where Charter translates to Vellum + declares its plugin-specific facts, and the host does the generic graph math — Phase 1 over-extended Charter by also shipping the _result_ of the walk (`child_ids`/`is_root`); Phase 3 corrected that by keeping the manifest channel but only carrying what Charter actually has an opinion about. A child referenced by multiple parents is rendered once per parent (DAG, not tree).

**Per-item `ops` (Phase 2 — plugin owns what's on the context menu, editor owns how to execute).** Each `PanelOp` is self-describing: `name` (the dispatch key the editor switches on), `label`/`icon` (what to render), and `kind` (an op-specific payload — today only `add-child` uses it to carry `"box"`/`"text"`/`"image"`). Charter's `common_item_ops` declares `rename`/`clone`/`lock`/`hide`/`deselect`/`delete` for every view; `container_item_ops` adds the `add-child` trio only when an item's `write_alias.is_some()` (Box with a `children` field — a Text/Image leaf gets no add-child ops, since it has nowhere to attach new children). `Views.svelte`'s context menu is built straight off `item.ops`, with the `lock`/`hide` labels overridden editor-side based on the current live DB row's `viewLocked`/`viewHidden` ("Lock" when unlocked → becomes "Unlock" when locked) — the manifest owns _availability_, the editor owns _current state + execution_. The editor dispatches each op via a switch on `op.name` calling the appropriate Manager API: `rename`→`api.renameView` (after triggering the in-place `Renameable` edit), `delete`→`api.deleteView`, `clone`→`api.cloneViewSubtree(viewId, write_alias ?? 'children')` then select the new id, `lock`→`api.toggleViewLock(viewId, !row.viewLocked)`, `hide`→`api.toggleViewHide(viewId, !row.viewHidden)`, `add-child`→`api.createViewInProject` + (for non-box kinds) `api.updateViewHints(newId, { charter: { primitive: kind } })` (primitive is a Charter hint, not a column — see Common Pitfalls), then `setViewChildren(parentId, [...current, newId])` to attach. Header ops (`header_ops`) are the same dispatch with no parent to attach to — they create top-level orphans that resolve picks up automatically. The `add-child` (box/text/image) sequence collapses into a submenu in the built menu. Adding a new op still requires editor support on the switch, but the plugin owns _which items offer which ops_ — that's the lock-in surface that moved out of the editor's hardcoded menu into the plugin's manifest.
...
**`taf_can_do/` is a separate repo with its own private remote (`origin` → `git@github.com:Variadic-Designs/vellum.git`, default branch `master`).** Its Rust source is committed and pushed **there**, not into `kit10` — only the compiled `vellum_renderer_bg.wasm` (and the generated JS/`.d.ts` glue) get committed into `kit10`. Push Vellum source changes to its own `origin` only; **never push to its `upstream` remote** (`Foxicution/wgpu-template`, the public template it was forked from). So a Vellum change is a two-repo commit: source → `taf_can_do` `origin/master`, rebuilt artifacts → `kit10`.
...
**`text_align`/`text_decoration` (text-affordances Phase 3) ride cosmic-text's own machinery rather than a hand-rolled implementation.** Alignment is `Buffer::set_text`'s existing `alignment: Option<Align>` parameter (`Left → None`, deliberately — that's cosmic-text's own pre-existing default, so a Left-aligned area's geometry is bit-for-bit unchanged from before this field existed); it only takes effect when `buffer.set_size` was called with a real width, which every post-layout `TextAreaData` has. Decoration is `Attrs::underline`/`.strikethrough()`, and `shape_area` reads the resulting per-span geometry back off `run.decorations` — **real font-metric-derived offset/thickness in EM units** (`GlyphDecorationData::underline_metrics`/`strikethrough_metrics`, straight from the font's own `post`/OS-2 tables via cosmic-text's `decoration_metrics`), not a guessed baseline offset. Screen-space conversion is `run.line_y - metrics.offset * font_size` uniformly for both (font-space +y = up flips to screen-space +y = down; verified against cosmic-text's own defaults — underline's negative offset lands below the baseline, strikeout's positive offset lands above it). Decoration quads are pushed into the **same glyph vertex buffer** as a new `mode = 2.0` (solid fill, no atlas sampling) — `glyph_shader.wgsl` gets a second `select` picking `input.color` outright for mode-2 vertices, keeping the existing unconditional `textureSample`/`fwidth` calls untouched (uniform control flow — see the derivative-op pitfall below). `hash_area` now hashes both fields: unlike `area.color` (applied post-cache), they change vertex generation itself, so a stale cache entry would keep rendering the old alignment/decoration.

**`line_height` (text-affordances Phase 4) replaces the hardcoded `font_size * 1.2` at both places that used it — `shape_area` (actual rendering) and `measure_text_node` (taffy's layout measure pass) — and the two MUST stay in agreement, or the box taffy sizes disagrees with what render actually draws.** `TextData`/`TextAreaData.line_height: f32` is always a resolved absolute px value; `≤ 0.0` is the "not provided" sentinel (mirrors the `font_weight` `if > 0 { .. } else { 400 }` idiom already used throughout `taf_can_do`), so an old Charter build that hasn't redeployed yet degrades gracefully to the previous ratio instead of a hard deserialize failure. `hash_area` and the taffy `TextMeasureKey` cache both now include it (changes glyph vertical position / `content_height`, not just post-cache state). `project_text_area` scales it by `view_zoom` identically to `font_size` — the `0.0` sentinel scales to `0.0` regardless of zoom, so the fallback survives projection correctly.

**Owns text measurement.** Text nodes arrive with `width: Auto, height: Auto`; Vellum measures them during the layout pass using cosmic-text constrained by available space from the parent. **`measure_text_node` answers taffy's three width probes distinctly:** `Definite(w>0)` wraps to `w`; `MaxContent` (and the defensive `Definite(≤0)` intrinsic-sizing probe) reports the natural unwrapped line; **`MinContent` reports the longest word** (shape at width 0, widest resulting line). This matters for flex: a flex item's automatic minimum size resolves to its min-content, so collapsing min-content into the full line (as an earlier version did) gave text-bearing children an automatic minimum equal to their whole width and they refused to shrink or wrap — they overflowed. `TextMeasureKey` carries a `width_mode` discriminant so min/max don't collide in the measure cache.

**`UiNode[]` contract:**

- Nodes are a flat array. Parent-child relationships use `parent_id: number | null` (index into the array).
- Size fields are `Extent` (`Auto | Px(f32) | Percent(f32)`), not bare `f32` (see the Plugin-Manager "Size fields" note). `Box{width: Auto, height: Auto}` → auto-size to children + padding; `max_*: Auto` → no constraint (not "max is 0px"); `min_*` is a lower clamp (Box only). `extent_dim` in `layout/mod.rs` maps each to taffy `Dimension` for `size`/`min_size`/`max_size`.
- `Text{width: Auto, height: Auto}` → Vellum measures during layout. Do not pre-patch text dimensions in the host.
- `flex_direction` on Box controls child stacking: `"Row"` | `"Column"` | `"RowReverse"` | `"ColumnReverse"`.
- `BoxExtra` also carries `align_items`/`justify_content`/`flex_wrap` (container), `flex_grow`/`flex_shrink`/`align_self`/`flex_basis` (item), and `margin` (uniform, all four sides) — full taffy fields, mapped 1:1 in `apply_box_extra` (`taf_can_do/src/layout/mod.rs`). `None` on the `Option`-typed ones means "don't touch it, leave taffy's own CSS-matching default" rather than "set to some zero value" — `flex_shrink: None` still ends up `1.0` (taffy's default), not `0.0`. `flex_basis: Option<Extent>` — `Px(0)` is what makes a "Fill" item an equal share of free space rather than content-size + leftover. **Charter no longer surfaces the item-level flex fields (`flex-grow`/`flex-shrink`/`align-self`) OR `margin` in the render panel** (margin is Charter's no-margins opinion; the item-flex trio is now driven by the Fixed/Hug/Fill "resize" control — see the Charter resize note below). Container-arrangement fields (`align-items`/`justify-content`/`flex-wrap`, plus `flex-direction`) now have their own opinionated control too — Charter's `arrange`/`compile_arrange` (Stack/Cluster/Split/Center/Grid), see the Charter arrangement note above. They're no longer top-level plain inputs; Charter still parses all of them from kit properties of the same CSS name as an escape hatch, surfaced via `arrange`'s per-tab "Advanced flex" disclosure instead.
...
vellum.ensure_index_visible(index) // frame the node at this index iff it's <85% visible; pans, and zooms OUT (never in) if the node is too big to fit at the current zoom; returns whether the camera moved
```
...
- **`parse_px` only handles pixel values; `parse_extent` is the sizing parser.** `parse_px` (used for `padding`/`gap`/`border-width`/`font-size`/etc.) silently returns `0.0` for `auto`/`%`/`em`. The box **sizing** properties (`width`/`height`/`min-*`/`max-*`) instead go through `parse_extent`, which understands `N%` → `Percent`, `Npx`/bare-`N` → `Px`, and `auto`/empty → `Auto`. `em` still isn't supported anywhere. Store hex/plain-number/`NNpx`/`N%` accordingly.
...
- **The `render_entries` constraint** (`value` or `token_id`, exactly one) means you cannot have a render entry with both a literal value and a token. A property's `viewRefs` (the view ids it references) come **only from a `view-list`-typed token** — the resolver reads them off the token's value type, never by parsing a literal (there is no literal-view-list path). The editor's `ChildViewField` always routes composition through a View-scoped `view-list` token anyway, because a view's child list is inherently per-instance data.

- **The resolver is name-neutral about composition — it does NOT know `children`.** A `view-list` token value is a native schema type (`TokenValueViewList`); the resolver carries the referenced ids as `ResolvedProperty.viewRefs` for **any** view-list-valued property, gated on the value _type_, never on a property _name_. There is no `childViewIds`, no `ResolvedKit`-level child list, no `property === 'children'` anywhere in `resolve.ts`. The opinion "`children` means nest these into a box" lives entirely in the plugin: **Charter** declares `composition_field_keys` (just `["children"]` today) on the panel manifest, and the **editor** walks `resolvedViews` for those keys' `viewRefs` to build the DAG host-side (`childrenByViewId`/`referencedViewIds`/`rootViews`). A different viewport plugin is never forced into Charter's `children` vocabulary — it would declare its own `composition_field_keys`.

- **The editor nests the Views tree off Charter's VIEW-INDEPENDENT composition keys, not the active view's categories.** The panel manifest carries `composition_field_keys` (Charter's fields whose inputType is the composition kind, across box **and** text categories — stable regardless of the active view), and the host walks `resolvedViews` for those keys to produce the DAG. This must **not** be derived from `fieldCategories` (which reflects only the active view's primitive) — doing so empties the keys and flattens the whole tree whenever a text/no-kit view is selected (e.g. when a freshly-cloned label becomes the newest, auto-selected view).
...
- **`cargo build` does not validate embedded WGSL.** Shader source is loaded as a plain string (`include_str!` or similar) and only actually compiled by wgpu at runtime in the browser — a syntax or type error in `.wgsl` files will build fine and only surface as a runtime failure. Validate with `naga` before shipping a shader change you can't visually test: a throwaway crate depending on `naga = { version = "...", features = ["wgsl-in"] }` (match the version already pinned in the target crate's `Cargo.lock`) calling `naga::front::wgsl::parse_str` then `naga::valid::Validator::validate` catches most of what wgpu itself would reject. **One class it does NOT catch: derivative operations (`fwidth`/`dpdx`/`dpdy`, and the implicit-LOD `textureSample`) in non-uniform control flow** — e.g. a `fwidth` call placed after an early `return` that's behind an `if`. naga passes it, but Dawn/WebGPU enforces the uniformity rule and rejects the whole pipeline at runtime (`[Invalid RenderPipeline]` → black screen). Keep every derivative call in uniform control flow (above any conditional `return`); compute derivative-derived values unconditionally at the top and `select()` between branch results, rather than branching around the derivative.
...
- **Don't gate inputType-specific widgets on `!isToken`.** A kit-default property (e.g. `src` on an image kit, `content` on a text kit) is backed by a kit-scope token — the resolved property carries `isToken: true`. Gating a custom widget (asset picker, font picker) on `!isToken` silently hides it for every kit-scoped default, which is the most common case. The `isToken` flag is irrelevant to how the value should be edited — it only signals that the render entry points at a token rather than a literal. The `content` field has the same property (always token-backed in the seed), but was never broken by this guard because it uses neither `suggestionsFrom` nor an `inputType`-specific widget — it falls through to the plain text input. Any new `inputType`-specific widget (asset picker, color picker, etc.) that targets a kit-scoped default field will hit the same bug if `!isToken` is in the condition. Check the `inputType` first, independently of the token state.
