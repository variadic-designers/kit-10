...
**Macro grid (`grid_shader.wgsl`, `fs_main`).** Sparse crosshairs at 16-world-unit intervals, drawn *first* in the content pass, right after the clear, so scene content paints over it. Opacity fades with `view_zoom` alone. Uses a `world_pos()` helper to convert `frag_coord` back to logical world space (needs `view_zoom * device_pixel_ratio`, see DPR-awareness above).
...
- **The `render_entries` constraint** (`value` or `token_id`, exactly one) means you cannot have a render entry with both a literal value and a token. `children` can be either — Charter/resolve.ts treat a token-backed `children` and a literal JSON-array `children` identically — but the editor's own `ChildViewField` picker always routes through a View-scoped `view-list` token (auto-created on first pick, updated in place after), never writes a literal, because a view's child list is inherently per-instance data. Only hand-authored seed data (`seed.ts`'s `setLayerChildren`) still uses the literal path.
