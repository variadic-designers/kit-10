...
| `on_resolve`          | Data change           | `OnResolveInput`                     | `OnResolveResult` (viewport + categories + view_icons) |
...
**The Views-tree icon is Charter's call too, not the editor's.** `on_resolve` returns `view_icons` (a `view_id → icon-class` map), built by running `primitive_icon(primitive_for_view(v))` over every project view — `primitive_icon` is paired 1:1 with `detect_primitive` (`text` → `fa-solid fa-italic`, else the box icon `fa-regular fa-window-maximize`). The editor stays agnostic — it never learns a view's primitive, it just renders whatever icon string the plugin hands back (`pluginManager.viewIcons` → `Views.svelte`'s `viewIcons[v.viewId]`, fallback `fa-regular fa-window-maximize`). The `viewLocked → fa-lock` override stays editor-side — that's editor state, not a primitive. Like `composition_field_keys`, `view_icons` rides on `on_resolve` only (a selection change never alters a primitive), and must stay snake_case (Charter-authored output the JS reads, see the camelCase pitfall below).
...
- **The resolver is name-neutral about composition — it does NOT know `children`.** A `view-list` token value is a native schema type (`TokenValueViewList`); the resolver carries the referenced ids as `ResolvedProperty.viewRefs` for **any** view-list-valued property, gated on the value _type_, never on a property _name_. There is no `childViewIds`, no `ResolvedKit`-level child list, no `property === 'children'` anywhere in `resolve.ts`. The opinion "`children` means nest these into a box" lives entirely in the plugin: **Charter** reads its own `children` field's `viewRefs` (`collect_child_view_ids` → `properties.get(CHILDREN_FIELD)`), and the **editor** nests the Views tree off whichever field the active plugin declares with `inputType: 'children'` (`viewCompositionKeys` in `Editor.svelte`, passed to `Views.svelte`) — the same field-kind indirection `suggestion-providers.ts` uses for fonts. A different viewport plugin is never forced into Charter's `children` vocabulary.

- **View-scope view-list tokens are self-declaring — a per-view composition override needs no render entry.** `applySelfDeclaredViewRefs` (run by both the batched and slow paths) materializes a property named after a view's own view-list token's **alias** (any alias, name-neutral), carrying its `viewRefs`, when no kit layer already declares that property. Only the **view's own** tokens self-declare — a kit/project-scope view-list token does not, so a kit default can never force a slot onto every view composing the kit (tested by `resolve.test.ts`'s self-declaring pair, which uses an arbitrary alias `slots` to prove name-neutrality). When a kit layer _does_ declare the property, the same view token overrides it by alias (view-scope wins). Consequence: composition is unconditional per view once a view-scope token exists (an instance override; see [[project_view_token_override_precedence]]) — conditional children are a kit/layer concern, not an instance one. **The seed declares no `children` render entry** for buttonKit; each button's labels come purely from its view-scope token.

- **Writing a per-view override (e.g. one view's `children`) goes through `api.upsertViewToken(projectId, viewId, alias, value)` — never through the resolved property's `tokenId`.** A View token overrides a same-named Kit/Project token during resolution (CONCEPTS.md §Tokens: "a View token overrides a Kit token of the same name"), matched by **alias** in `resolve.ts`'s `viewListTokenMap`/`tokenMap`. A resolved property's `tokenId` is instead the _declaring render entry's_ token — for a kit-declared property that's a shared kit-scoped base, so writing it either moves every view at once or is silently shadowed. `upsertViewToken` (find-or-create the view-scoped token for the alias, in one transaction) is the whole write path — used by **both** `ChildViewField.svelte` and the Views panel's drag-and-drop nesting (`Views.svelte`'s `setViewChildren`), and needs no render entry (children self-declare, above). This bug hit both call sites before they shared the helper.

- **Kit default children (clone-per-view / component-instance model).** A kit ships default children as a **kit-scope `view-list` token** (the template subtree, e.g. seed's `Button` kit → `Label: Default`). It has **no render entry**, so it never resolves into a shared child on its own — it's read directly by `api.instantiateKitDefaults(viewId)`, which fires eagerly when the editor composes a kit onto a view (`Compose.svelte`'s `attachKit`). For every kit-scope view-list token (any alias — name-neutral), it **deep-clones** the template subtree (`api.cloneViewSubtree`: view + compositions + axis args + view-scope tokens, recursing the same-aliased view-list refs, DFS cycle guard) into fresh per-instance views and writes them as the view's own same-aliased view-scope token. So each instance owns **unique** child views (uniqueness preserved — a view is referenced as a child at most once), and the template views stay top-level editable **masters** (referenced by no instance's resolved refs → not nested). Idempotent (skips aliases the view already overrides). Cloning is a pure DB op, **never** in `resolve.ts`. Tested in `api.test.ts` (uniqueness + recursion + idempotence).

- **The editor nests the Views tree off Charter's VIEW-INDEPENDENT composition keys, not the active view's categories.** `on_resolve` returns `composition_field_keys` (Charter's fields whose inputType is the composition kind, across box **and** text categories — stable regardless of the active view), surfaced as `pluginManager.compositionFieldKeys` and passed to `Views.svelte` as `viewCompositionKeys`. This must **not** be derived from `fieldCategories` (which reflects only the active view's primitive) — doing so empties the keys and flattens the whole tree whenever a text/no-kit view is selected (e.g. when a freshly-cloned label becomes the newest, auto-selected view).
