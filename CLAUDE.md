...
**A squircle box exports as a proportionally-SCALED circular `border-radius`, not its literal `corner_radius`.** Real CSS's `corner-shape` property IS a genuine companion to `border-radius` (not a competing/unrelated mechanism) - `border-radius` still controls the corner box's size, `corner-shape: superellipse(n)` controls the curve drawn inside it - but as of mid-2026 it's Chromium-only (Chrome/Edge 139+, ~65% of global users), not Baseline, with no public Firefox/Safari timeline, so it isn't usable as the only mechanism for a "just works everywhere" static export. `clip-path: path(...)` is broadly supported but clips the whole element without drawing a matching border/shadow along the new silhouette, needing extra per-node markup for a fallback whose whole point is staying simple - deliberately out of scope. **A `@supports (corner-shape: superellipse(4))` progressive-enhancement block rides alongside every squircle rule** (`css.rs::squircle_supports_block`, shared by both paths) - a supporting browser gets the LITERAL radius + real `corner-shape: superellipse(4)`, overriding the scaled fallback via ordinary same-specificity cascade (no `!important` needed); a non-supporting browser skips the whole block via feature detection. Always the explicit `superellipse(4)`, never the bare `squircle` keyword, since that keyword is shorthand for `superellipse(2)` (a different, rounder curve than this codebase's `SQUIRCLE_N = 4`) and would visually diverge from the fallback it's meant to enhance. Path B's per-variant override (`variants::VariantRule.squircle_radius`) only fires when that SPECIFIC variant's own delta actually redeclares `border-radius` - an untouched variant needs no override of its own, since CSS cascades per-declared-property, and the base rule's own `@supports` block already covers it. Contested cross-kit `border-radius` rules are a deliberate scope cut - no `@supports` override there, just the already-correct scaled fallback (see `resources/webcodium-export-plan.md`). "Same px value" is not actually the closest match: a circular corner at radius `r` removes a fixed ~21.5% of its own `r×r` corner-square area (`1 - π/4`), while Vellum's shipped `n=4` squircle removes a fixed ~7.3% - both ratios independent of `r`. `SQUIRCLE_AREA_MATCH_SCALE` (`css.rs`, `≈0.583`, `pub(crate)` so `variants.rs` shares the exact same literal rather than a second hand-typed copy) is the radius multiplier that makes the fallback circle remove the same ABSOLUTE corner area the squircle did - self-verified by a numeric-integration test (`squircle_area_match_scale_is_self_consistent`) so a hand-edit without re-deriving it fails loudly. Path A (`css.rs`'s `paint_props`) reads `BoxData`/`TextData.squircle` directly; Path B (`variants.rs`) has no such field to read (raw kit properties only), so `synthesize_radius` consumes the boolean `border-radius-squircle` companion property and scales `border-radius` in place before the normal diffable-property pipeline runs, mirroring `synthesize_border`'s "merge two raw properties into one atomic declaration" shape exactly. **Coupling warning**: this constant is mathematically derived FROM `SQUIRCLE_N = 4` in `taf_can_do/src/render/shader.wgsl` - if that shader constant is ever retuned, this must be recomputed too, with no automated cross-language link enforcing it.
...
**Text-on-path is authored via `text-path` (+ optional `text-path-offset`), a raw JSON-array escape hatch** (`[[x, y], ...]`, box-local space - same origin/units convention as `ShapeKind::Path`'s segments), same posture as `grid-template-areas`/custom grid tracks: no dedicated point-picker widget yet. `build_text_node`'s `parse_text_path` treats fewer than 2 points or unparseable JSON as unset, never an error. Compiles to `kit10-scene`'s shared `Deform` enum (`resources/foundation.md` Pillar C, the `p -> p'` contract also reserved for future SDF domain-warp/envelope work) - only `Deform::ArclengthPath` is interpreted anywhere today. Vellum bends each glyph's quad individually (`shape_area`'s `glyph_path_transform`): samples a `kit10_motion::Path` built from the points at that glyph's own pen-center arc-length position, reusing `GlyphTransform`'s rotate-about-pivot-then-translate affine with the sampled tangent/point - the same "transform the quad, never reshape it" primitive the whole-area dynamic layer (Pillar A) already uses, just per glyph. A decoration span (underline/strikethrough) gets one shared pivot at its own horizontal center - an approximation, not curve-exact. WebCodium does not export text-on-path - no cheap real-CSS equivalent for an arbitrary path bend, a documented v1 scope cut.
...
**Primitive detection (`detect_primitive`):** a `src` property → `"image"`; a `kind` property → `"shape"`; text-only properties → `"text"`; otherwise → `"box"`, recursing into `child_view_ids`. **`has_box_props` must only include genuinely structural properties** (`width`, `height`, `display`, `flex-direction`, `gap`, `grid-template-*`) - never fill/border/radius/padding, which real CSS text can have without stopping being text. That used to silently swap a node's whole sizing algorithm the moment a label got a background color.

**`Shape` is the fourth render primitive** (`resources/shapes-drawing-plan.md`, Phase 1 shipped 2026-08-06): a parametric vector shape (`ShapeKind::Rect | Ellipse | Line | Polygon{sides} | Star{points, inner_ratio}`), sized like Box (mirrors width/height/min/max, including Fixed/Hug/Fill via `compile_resize`) but always a **leaf** - no arrange/grid-container opinion, no children, same posture as Img. `build_shape_node`/`shape_categories()` (Charter) and `CREATABLE_PRIMITIVES`'s `"shape"` entry follow the same one-canonical-table pattern the box/text/image trio already established, so nothing about Shape needed hand-duplicating across `container_item_ops`/`views_header_ops`/the editor's add-child dispatch. `fill`/`stroke`/`stroke-width` are ordinary resolvable properties (tokenizable, Layer-conditionable), never baked. WebCodium (`plugins/webcodium/`) does not export Shape yet - `node_props`/`render_html_node` return nothing for it, a documented v1 scope cut (no cheap real-CSS equivalent for polygon/star), not a bug.
...
- Size fields are `Extent` (`Auto | Px(f32) | Percent(f32)`), not bare `f32`. `Box{width: Auto}` auto-sizes to children+padding; `max_*: Auto` means no constraint; `min_*` is a lower clamp (Box, Shape, and SpriteBatch only - both Shape and SpriteBatch mirror Box's own sizing box exactly).
- `Shape` (`ShapeData`) is a parametric vector primitive - `kind: ShapeKind`, `fill`/`stroke`/`stroke_width`, `extra: BoxExtra` (flex-item participation only, always a leaf). Rendered by its own analytic-SDF pipeline (`shape.wgsl`/`ShapeInstance`), never the rounded-rect `RectInstance` path - `layout::node_rect` gives it an invisible hit-test/selection bbox rect (same treatment as `Img`) so selection/hover/drag/hit-test work for free, but visual paint is entirely separate. `sd_polygon_generic` (a generic simple-polygon edge-iteration SDF, exact for both a regular n-gon and a non-convex star silhouette) backs Polygon/Star; stroke on any kind reuses one shared trick - eroding a true SDF by a constant is just `d + stroke_width` - so `composite_border` from `shader.wgsl` applies unchanged.
...
**Selection is a separate overlay, not a border override.** `rebuild_selection_instances`, run every frame from `write_frame` (not just on scene change), turns captured layout rects into an outline `RectInstance` + 4 corner handles, expressed in world units as `desired_screen_px / view_zoom` so they stay a constant screen size regardless of zoom. Runs every frame because pan/zoom mutators update the camera without touching the instance buffer. **The outline itself mirrors the target's own `squircle` corner style** (`SelectionDecoration`/`HoverDecoration.squircle`, threaded from `LayoutRect.squircle`) - a circular outline around a squircle-cornered box visibly failed to hug the curve. The 4 corner **handles** stay circular unconditionally regardless - they're fixed small UI chrome dots, not a shape being traced.
...
**The dynamic-animation layer (foundation.md Pillar A/B/C/D-a) is Vellum-side, additive, and inert until driven** - it never touches Charter or the resolve pipeline, the same "dynamic vs structural" split `resources/foundation.md` argues. A per-node transform (translate/scale/rotate/opacity) is applied at INSTANCE-BUILD time via a per-frame `render_affines: HashMap<node_index, ResolvedAffine>` (flattened onto an animated node's subtree), consumed by `to_instances` (boxes), the image + glyph builders, and the selection/hover overlay rebuild - so every primitive AND its chrome follow one transform. Rotation is a real per-instance attribute threaded through the box + image vertex shaders (the fragment SDF stays in unrotated local space, so corners stay exact); text rotates/scales per-glyph via `text/mod.rs`'s `GlyphTransform` (transforms the screen quad, NO reshape - the atlas cell is untouched). Four runners feed the layer, all advanced in `draw()` and reusing `kit10-motion`: `step_tweens` (GSAP-ease tweens), `step_motion_paths` (a node along a `kit10-motion::Path` by arc-length, autoRotate to the tangent), `step_flip` (Flip: interpolate a node per-index between a captured "first" layout and the current one - `begin_flip`/`commit_flip`), plus direct `set_node_dynamic`. `is_animating()` gates the JS/native rAF drive; `set_scene` supersedes all of it. **Any per-frame system reading `render_affines` also needs `scene_version` bumped so the glyph buffer re-projects** (same cache gate `update_node_drag` defeats) - `refresh_render_affines`/`step_flip` do this. Text-on-path (per-glyph path placement) and the `Deform` wire contract are the remaining pieces (foundation-execution.md M5). The `Shift+T`/`Shift+F`/`Shift+M` Viewport keybinds and the `debug_*`/`demo_*` Graphics methods are temporary verification scaffolding.
...
**Paint order is NOT depth-sorted by tree/z-position - one global pass per primitive type** (all boxes, then all shapes, then all images, then all text). Any text anywhere always paints above any box anywhere, regardless of nesting. Rarely noticeable until views overlap (e.g. via drag). Not yet fixed; would need interleaved per-node draws (loses batching) or a depth buffer.
...
- **The kit-DEFAULT-children mechanism is a separate, narrower thing from the view-instance composition above, and IS hardcoded to the literal property name `"children"` - by design, not an oversight.** `setLayerChildren`/`clearLayerChildren`/`remapRenderEntryValue` (`manager/src/api/index.ts`) write and remap a Layer's own `children` render entry as a literal JSON-array-of-view-ids string (consumed by `instantiateKitDefaultsImpl` when a kit is first attached to a view), never the `view`-typed-token mechanism above. Unlike the resolver/editor DAG walk, these three functions do NOT derive the alias set from Charter's `composition_field_keys` - they assume exactly one composition alias exists project-wide, which is true today (Charter declares exactly one `input_type: "children"` field). If a second composition-alias field is ever declared, these three functions need matching updates or a second alias's kit defaults will silently fail to import/remap correctly.
