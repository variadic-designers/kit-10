...
> **Status: Phases 1–4 shipped; Phase 5 satisfied by construction.**
> `arrange`/`compile_arrange` (Stack/Cluster/Split/Center/Grid tabs + inline
> submenus + per-tab Advanced disclosure), the `spacing` inputType (numeric
> stepper + padding's 1↔4 CSS-shorthand affordance), Grid's real `Cell min +
...
> Phase 5's ask (cascade-scope color on layout controls) fell out of
> `ArrangeField`/`ResizeField` reusing `StyleField` for every row - the
> header track dot and each follow-on already carry the layer-source hue;
> only per-tab-button coloring would be extra, if ever wanted.
...
layout _intents_ that nudge users into the handful of patterns real UIs are actually
made of."

Grounding facts it builds on: Charter is an _opinionated translation layer_, not a
...
is _debt_ - "acceptable as a placeholder for a primitive that has no clearer opinion
_yet_, but the north star is to earn an opinion and replace it." The `layout` category
...
- grid tracks" is the wrong surface isn't only that the names are CSS - it's that they
  make the user _assemble_ a pattern out of orthogonal primitives every time, when the
...
_recognize their pattern by name_ and pick it, not reverse-engineer it from flex
...
The tab row quietly _is_ `display` plus most of the flex quartet, renamed for
...
the parent's main axis," and now the _parent's_ main axis is whatever its Arrangement
...
widget whose _meaning_ changes with a mode you can't see** (its 3×3 grid means
...
1. **The tab row is the signifier.** The submenu changes only as the direct, visible
   ...
   in the same physical location, so users can't tell _why_ a control is missing").
...
4. **Hidden means inert _and_ preserved.** Follow-ons are ordinary render entries.
...
- _Payoff:_ the single densest cluster of raw-CSS debt becomes one outcome-named
...
- _Payoff:_ the highest-cliff part of the category becomes a two-knob common case.
...
- _Payoff:_ four permanent raw fields become contextual, and their purpose becomes
...
- Arrangement now crosses into structural behavior that resolves across kits × axes ×
  ...
  blast radius _before_ committing. No layout control should mutate shared state with
  no cue to its scope.
- That includes the tab row itself: `arrange` is a resolved property like any other,
  so the active tab carries the source hue for _which layer chose this arrangement_ -
...
and only _surface_ an opinion once it's genuinely settled. Until each phase lands, its
...
1. **Name for the outcome, not the property.** _Stack, Cluster, Split, Center, Grid_ -
...
3. **One field, one meaning.** Tabs change which fields _exist_, never what a visible
   field _means_ - the inverse of Figma's mode-overloaded widgets.
