...
- Rebuild **both** wasm artifacts and copy: Vellum via the `wasm-pack ... --no-default-features --no-opt` command (CLAUDE.md), Charter via `cargo build --target wasm32-unknown-unknown --release` → `static/charter.wasm`.
