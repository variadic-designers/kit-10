...
// (BoxData/TextData/ImgData/ShapeData) - the machine-truth source, generated by the kit10-scene
// crate. Used to guard that PLUGINS.md's node examples document every real field, so the
// illustrative JSON can't silently fall behind the actual wire (the exact drift that let
...
export function nodeFieldsFromSchema(schemaJson) {
	...
	const byVariant = { Box: 'BoxData', Text: 'TextData', Img: 'ImgData', Shape: 'ShapeData' };
...
export function renderWireTypes(schemaJson) {
	const defs = JSON.parse(schemaJson).$defs || {};
	const first = ['BoxData', 'TextData', 'ImgData', 'ShapeData'];
